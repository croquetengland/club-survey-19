...
`r nrow(df_clean)` of 210 Full and Affiliate clubs (`r scales::percent((nrow(df_clean))/210)`) submitted a return, representing `r tbl_membership_by_fed$NumPlayers %>% last()` total members: `r tbl_membership_by_fed$NumRetired %>% last()` retired, `r tbl_membership_by_fed$NumU25s %>% last()` U25 and `r tbl_membership_by_fed$NumUnique %>% last()` unique members (excluding 2nd club members). 
...
```{r, echo = FALSE}
tbl_membership_by_fed %>% 
  select(-Clubs) %>% 
  head(9) %>% 
  gather(key,val, -Federation) %>% 
  mutate(key = fct_reorder(key, desc(val))) %>% 
  ggplot(aes(x = Federation, y = val, fill = key)) + 
...
DT::datatable(tbl_membership_by_fed %>% 
                mutate(`%Unique` = scales::percent(NumUnique/NumPlayers, accuracy = 1),
                       `%Retired` = scales::percent(NumRetired/NumPlayers, accuracy = 1),
                       `%Active` = scales::percent(NumActive/NumPlayers, accuracy = 1),
                       `%U25` = scales::percent(NumU25s/NumPlayers, accuracy = 1)
                       ) %>% 
                select(1:3,9:12),
              options = list(dom = 't'))
```
...
Most clubs have a handicapper of some sort with only `r tbl_handicappers %>% adorn_totals %>% tail(1) %>% pull(wo_HCer)` without (`r scales::percent(sum(tbl_handicappers$wo_HCer)/nrow(df_clean))`). There are a reasonable number of registered handicappers throughout the Federations.
...
In total, including the unresolved issues above, we had **`r nrow(df_clean)` returns out of 210 Full/Affiliate clubs, a return rate of `r scales::percent((nrow(df_clean))/210)`**.

Of the clubs who did not respond, most are small. Notable non-respondents were Camerton & Peasedown, Hampstead Heath, and Swindon. No analysis has been completed on why they did not respond - it may be worth a quick call for Council Fed reps to follow up.

```{r echo = FALSE, warning = FALSE, message = FALSE}
DT::datatable(readr::read_csv('data/nonresponses.csv'))
```
...
DT::datatable(df_incorrect_court_count, options = list(dom = 't'))
```
...
```{r membership-percentage-full, echo=FALSE, warning=FALSE}
...
tbl_GC_vs_AC_by_fed %>% 
  ...
  gather(key, val, -Federation) %>% 
  group_by(Federation) %>% 
  mutate(label = scales::percent(val/sum(val))) %>% 
  ungroup() %>% 
  ggplot(aes(x = fct_reorder(Federation, desc(val)), y = val, fill = key, label = label)) + 
...
DT::datatable(tbl_GC_vs_AC_by_fed %>% 
                mutate(`sum(cols3-5)` = NumGCOnly + NumACOnly + NumBoth) %>% 
                select(1:6,8,7), options = list(dom = 't'))
```
...
```{r, echo = FALSE}
DT::datatable(tbl_GC_only_by_fed, options = list(dom = 't'))
```
...
```{r, echo = FALSE}
DT::datatable(tbl_taking_up_ac, options = list(dom = 't'))
```
...
```{r, echo = FALSE}
DT::datatable(tbl_enters_CA_events, options = list(dom = 't'))
```
...
Analysis of responses to be completed.
